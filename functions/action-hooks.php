<?php
/*
 * mini-bootstrap5 Action Hooks
 * =================
 * Designed to be used by a child theme, but they can also be used directly 
 * in your development of mini-bootstrap5. Example usage:
...
function b5st_navbar_search() {
  if ( ! has_action('navbar_search') ) {
    ?>
    <form class="md-flex" role="search" method="get" id="searchform" action="<?php echo esc_url( home_url( '/' ) ); ?>">
      <div class="input-group">
        <input class="form-control border-secondary" type="text" value="<?php echo get_search_query(); ?>" placeholder="搜索..." name="s" id="s">
        <button type="submit" id="searchsubmit" value="<?php esc_attr_x('Search', 'mini-bootstrap5') ?>" class="btn btn-outline-secondary">
...
function b5st_bottomline() {
    ...
    <div class="container-xxl">
      <div class="row">
        ...
        <div class="col-sm">
          <p class="text-center text-sm-end">Based on <a href="https://github.com/SimonPadbury/b5st">b5st</a> & Modified by yeni</p>
        </div>
      </div>
    </div>
		<?php		
